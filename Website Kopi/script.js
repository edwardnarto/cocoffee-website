...
document.addEventListener('DOMContentLoaded', () => {
    ...
    checkoutBtn.addEventListener('click', () => {
        if (cart.length > 0) {
            ...
            alert('Proses pembayaran dilanjutkan! Total item: ' + cart.length + '. Total belanja: ' + formatRupiah(calculateTotal()));
            
            // Bersihkan keranjang
            cart = []; 
            updateCartCount();
            cartModal.classList.remove('show');
            showToast('Checkout berhasil!', true);
        } else {
            alert('Keranjang masih kosong! Silakan tambahkan minuman.');
        }
    });
});
